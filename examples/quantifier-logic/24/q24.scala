...
val HilbertSentences = Seq(
  fof"∃x ∀y Less(x, y)", // Could this ever be true?
  fof"∀x (Cir(x) → ∃y Less(x, y))",
  fof"∃x (Cir(x) ∧ ∀y Less(x, y))",
  fof"∃y ∀x (Cir(x) → Less(x, y))", // Try to say in English. Hard to say unambiguously.
  fof"∃y ∀x (Cir(x) → Less(y, x))",
