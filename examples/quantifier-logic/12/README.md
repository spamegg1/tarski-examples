...
15. Something is a square if and only if
    it is not in the same column as either `a` or `b`.
...
- Adjust `VonNeumannWorld` by moving `a` directly below `c`.
...
Hungarian-American mathematician, computer scientist, engineer (1903 - 1957)

Major contributions to many fields, worked on the Manhattan Project.
